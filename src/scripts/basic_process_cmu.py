...
parser.add_argument(
    ...
    type=str,
    default="./scripts_json_helpers/process_movies_helper.json",
    help="(by default it is in the same folder)",
)
parser.add_argument(
    "--process_actors_helper",
    type=str,
    default="./scripts_json_helpers/process_actors_helper.json",
    help="(by default it is in the same folder)",
)
parser.add_argument(
    "--process_ethnicities_helper",
    type=str,
    default="./scripts_json_helpers/process_ethnicities_helper.json",
    help="(by default it is in the same folder)",
)
parser.add_argument(
    "--process_type", choices=["all", "movies", "characters"], default="all"
)
...
if args.process_type in ["all", "movies"]:
    ...
    movie_raw["languages"] = movie_raw["Movie languages"].apply(
        lambda x: process_languages(
            x, skip_languages=set(movies_helper["skip_languages"])
        )
    )
    print("[INFO] Movie: languages are processed")

    movie_raw["movie_release_date"] = pd.to_datetime(
        movie_raw["Movie release date"].apply(
            lambda x: process_date(x, change_date=movies_helper["change_date"])
        )
    )
    print("[INFO] Movie: releases dates are processed")

    movie_raw["countries_old"] = movie_raw["Movie countries"].apply(
        lambda x: process_countries(
            x, rename_countries=movies_helper["rename_countries"]
        )
    )
    print("[INFO] Movie: countries are processed")

    movie_raw["countries"] = movie_raw["countries_old"].apply(
        lambda x: process_countries_old2new(
            x, rename_countries=movies_helper["old_to_new"]
        )
    )
    print("[INFO] Movie: old2new countries are processed")

    movie_raw["genres"] = movie_raw["Movie genres"].apply(
        lambda x: process_genres(x, rename_genres=movies_helper["rename_genres"])
    )
    print("[INFO] Movie: genres are processed")

    movie_raw = movie_raw.drop(
        columns=[
            "Movie release date",
            "Movie languages",
            "Movie countries",
...
if args.process_type in ["all", "characters"]:
    ...
    char_raw = pd.read_csv(
        os.path.join(args.data_dir, "character.metadata.tsv"), sep="\t", header=None
    )
    char_raw.rename(
        columns={
            0: "Wikipedia movie ID",
    ...
    char_raw["actor_date_of_birth"] = char_raw["Actor date of birth"].apply(
        lambda x: process_date(x, change_date=actors_helper["change_date"])
    )

    print("[INFO] Characters: (Actor date of birth) are processed")

    char_raw["movie_release_date"] = char_raw["Movie release date"].apply(
        lambda x: process_date(x, change_date=movies_helper["change_date"])
    )
    ...
    char_raw = char_raw[
        char_raw["actor_date_of_birth"] < char_raw["movie_release_date"]
    ].copy()
    ...
    if os.path.exists(args.process_ethnicities_helper):
        ...
        all_ethnicities_found = (
            set(list(ethnicity_process_helper["fb2ethnicity"].keys())) & all_ethnicities
        )
        ...
        for ethn_id in all_ethnicities_found:
            ...
            result["race"] = ethnicity_process_helper["ethnicity2race"][
                result["ethn_name"]
            ]
            ethnicity_pd.append(result)
        ethnicity_pd = pd.DataFrame(ethnicity_pd)
        char_raw = char_raw.merge(
            ethnicity_pd,
...
if args.process_type in ["all", "characters"]:
    char_raw.to_csv(
        os.path.join(args.output_dir, "character_processed.csv"), index=False
    )
